...
interface StaffState {
  ...
  updateSalary: (
    ...
    overTimeRate?: number
  ) => Promise<{ status: string; message: string; updates?: any[] } | undefined>;
}
...
export const useStaffStore = create<StaffState>()(
  persist(
    ...
    {
      name: "staff-storage",
      // Only persist specific fields, not loading/error states
      partialize: (state) => ({
        yourStaffProfileInWorkplace: state.yourStaffProfileInWorkplace,
        attendance: state.attendance,
        dailyRecords: state.dailyRecords,
        staffList: state.staffList,
        attendanceCompany: state.attendanceCompany,
        dailyRecordCompany: state.dailyRecordCompany,
      }),
    }
  )
);
